...
class Graph(Base):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # calculate scale
        self.minx = min(min(x1), min(x2 or (sys.maxsize,)))
        self.maxx = max(max(x1), max(x2 or (-sys.maxsize,)))
        self.miny = min(min(y1), min(y2 or (sys.maxsize,)))
        self.maxy = max(max(y1), max(y2 or (-sys.maxsize,)))
