...
class Gantt(Base):
    ...
    def __init__(self, data, **kwargs):
        ...
        # create name column
        def draw_names():
            size = self.textSize('Name')
            maxwidth = size.width
            maxheight = 0.
            
            textdata = []
            for duration in data:
                text = str(duration.name)
            ...
            main.EText(font, 'Name', x = xpos, y = ypos, stroke='none')
            y += delta
            maxheight += delta
        ...
        def draw_duration(x):
            ...
            ypos = y + .5*size.height - .5*size.descent
            main.EText(font, 'Duration', x = xpos, y = ypos, stroke='none')
        ...
        def plot_month(x, start, end):
            ...
            while year <= endyear:
                ...
                s = "%s %d" % (calendar.month_abbr[month], year)
        ...
        def plot_items(x, start, end):
            ...
            for duration in data:
                ...
                color = duration.color or next(colors)
