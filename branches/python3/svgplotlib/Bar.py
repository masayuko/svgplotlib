...
class Bar(Base):
    ...
    def __init__(self, values, labels = None, colors = None, **kwargs):
        ...
        # calculate scale
        miny = self.miny = min(min(y1), min(y2 or (sys.maxsize,)))
        maxy = self.maxy = max(max(y1), max(y2 or (-sys.maxsize,)))
        ...
        for idx, value in enumerate(values):
            ...
            plotArea.Rect(x = x, y = y, width = barWidth, height = barHeight, fill = next(color))
